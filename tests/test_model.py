...
def _make_dataframe(n: int = 100, seed: int = 42) -> tuple[pd.DataFrame, pd.Series]:
    ...
    y = pd.Series(
        ((df["geopolitical_risk"] > 0.6).astype(int) | (df["defect_rate"] > 0.10).astype(int)).clip(0, 1)
    )
    return df, y
...
class TestPrediction:
    ...
    def test_predict_returns_risk_score(self, trained_pipeline):
        sample = {
            "lead_time_days": 30, "on_time_rate": 0.9, "defect_rate": 0.01,
            "financial_score": 0.9, "geopolitical_risk": 0.2,
            "capacity_utilization": 0.5, "years_active": 10,
    ...
    def test_high_risk_supplier_gets_higher_score(self, trained_pipeline):
        low_risk = {
            "lead_time_days": 14, "on_time_rate": 0.98, "defect_rate": 0.005,
            "financial_score": 0.95, "geopolitical_risk": 0.1,
            "capacity_utilization": 0.4, "years_active": 20,
            "is_sole_source": 0, "country": "US", "category": "logistics",
        }
        high_risk = {
            "lead_time_days": 90, "on_time_rate": 0.55, "defect_rate": 0.14,
            "financial_score": 0.35, "geopolitical_risk": 0.92,
    ...
    def test_predict_country_variants(self, trained_pipeline, country):
        sample = {
            "lead_time_days": 30, "on_time_rate": 0.8, "defect_rate": 0.05,
            "financial_score": 0.7, "geopolitical_risk": 0.4,
            "capacity_utilization": 0.6, "years_active": 5,
            "is_sole_source": 0, "country": country, "category": "electronics",
        }
        result = predict(sample, trained_pipeline)
        assert 0.0 <= result["disruption_risk"] <= 1.0
...
class TestReorderPoint:
    @pytest.mark.parametrize("mean,std,lead,z,expected_min", [
